// Prompt compartilhado entre POST /api/generate-script (Script Builder, owner)
// e a geração automática semanal (lib/script-intelligence/weekly-suggestion.ts).
// Extraído para módulo próprio para as duas chamadas usarem exatamente o
// mesmo texto — divergência aqui faria o script gerado pelo cron ter uma
// estrutura/qualidade diferente da gerada manualmente pelo owner.

export const SYSTEM_PROMPT = `You are a sales script architect for a dog training business. Analyze the provided call transcripts and/or text and reorganize/rewrite that material into a structured sales script split across exactly 5 fixed sections, in the exact order and with the exact meaning defined below.
...
export function buildUserPrompt(transcripts: string[], textInput: string | null): string {
  const parts: string[] = []
