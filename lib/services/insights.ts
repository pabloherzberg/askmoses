...
function buildInsightsFromData(trainers: Trainer[], calls: Call[]): Insight[] {
  ...
  const trainerWeakestScores = new Map<string, { sum: number; count: number; name: string }>()
  for (const call of calls) {
    if (!call.trainerId) continue
    if (!trainerWeakestScores.has(call.trainerId)) {
      trainerWeakestScores.set(call.trainerId, { sum: 0, count: 0, name: call.trainerName })
    }
  ...
  const closeRateLow = callsWithLow.length > 0
    ? Math.round((callsWithLow.filter((c) => c.result === 'closed').length / callsWithLow.length) * 100)
    : 0
  const closeRateHigh = callsWithHigh.length > 0
    ? Math.round((callsWithHigh.filter((c) => c.result === 'closed').length / callsWithHigh.length) * 100)
    : 0
  ...
  const atRisk = withDrop.length > 0
    ? withDrop.reduce((worst, t) => t.scoreDelta < worst.scoreDelta ? t : worst)
  ...
  const atRiskCloseRate = atRiskCalls.length > 0 ? Math.round((atRiskClosed / atRiskCalls.length) * 100) : 0
  ...
  const avgClose = sorted.length > 0
  ...
  insights.push({
    ...
    action: 'Keep the cadence. Consider daily uploads for faster feedback loops.',
  })
