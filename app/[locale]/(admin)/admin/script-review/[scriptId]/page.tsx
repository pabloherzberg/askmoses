...
function buildReviewData(
  ...
  const originalSections: ScriptSection[] = Array.isArray(script.sections)
    ? script.sections
    : [];
  const originalCriteria: ScriptCriterion[] = Array.isArray(script.criteria)
    ? script.criteria
    : [];
  ...
  const reviewSections: ReviewSection[] = aiResult.sections.map((sec, i) => {
    ...
    const changed =
      !orig || orig.instructions !== sec.instructions || orig.tips !== sec.tips;
    return {
      name: sec.name,
      ...
      weight:
        sec.weight ??
        orig?.weight ??
        Math.round(100 / Math.max(aiResult.sections.length, 1)),
      ...
      previous:
        changed && orig
          ? { instructions: orig.instructions, tips: orig.tips }
          : null,
    };
  });
...
export default async function ScriptReviewDetailPage({ params }: PageProps) {
  ...
  if (!improveRes || !improveRes.ok) {
    const { buildMockImprovement } =
      await import("@/lib/scripts/mock-improvement");
