...
- ✅ **0.3** — Confirmar que os consumidores de média passam a excluir `scoring_status IN ('scoring_failed','transcript_leaked')`. Pontos: [lib/db/trainers.ts](../lib/db/trainers.ts) e [scripts/107](../scripts/107_won_rate_and_weekly_stats.sql) (filtro na CTE `base`). **Concluído em 24/09/2026** — migrations 110 e 107 aplicadas em produção pelo usuário; recálculo retroativo forçado via `stamp_call_stats_weekly(p_since => '2026-09-17')` (84 semanas verificadas, 8 linhas atualizadas com os números já sem as calls marcadas).

> **Nota:** o próximo número livre é **109** — 108 é o último. Números colidem em branches paralelas no histórico (020, 043, 107…); confirmar antes de commitar.
